...
const getUserController = async (req:Request,res:Response)=>{
        try {
            ...
            const identify = req.body.auth.phone;
            console.log("Payload : "+JSON.stringify(req.body.auth,null,2));
            const userRepo = AppDataSource.getRepository(User);
            const userData = await userRepo.findOne({where : {phone:identify}});
...
//             const userRepo = AppDataSource.getRepository(User);
//             const user = await userRepo.findOne({where : {phone:identify}});
...
const updateUserController = async(req:Request,res:Response) => {
    try {
        ...
        if(!data.password || !data.phone){
            const toUpdate = req.params.userId;
